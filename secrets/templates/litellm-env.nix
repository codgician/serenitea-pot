{ ref, pubkeys, ... }:
{
  ...
  content = ''
    ...
    GENERIC_CLIENT_SECRET=${ref "litellm-generic-client-secret"}
    PROXY_ADMIN_ID=${ref "litellm-proxy-admin-id"}
    UI_USERNAME=${ref "litellm-proxy-admin-id"}
    UI_PASSWORD=${ref "litellm-ui-password"}
    AZURE_AKASHA_API_KEY=${ref "litellm-azure-akasha-api-key"}
    GEMINI_API_KEY=${ref "gemini-api-key"}
    DEEPSEEK_API_KEY=${ref "litellm-deepseek-api-key"}
    NVIDIA_NIM_API_KEY=${ref "litellm-nvidia-nim-api-key"}
    ANTHROPIC_API_KEY=${ref "litellm-anthropic-api-key"}
